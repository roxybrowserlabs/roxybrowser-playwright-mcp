import { describe, expect, it } from "vitest";
import { withBidiPage } from "../../../helpers/bidi.js";

describe("page click scroll contract e2e (bidi/firefox)", () => {
  it("should not scroll the page when scroll is \"none\"", async () => {
    await withBidiPage(async (page) => {
      await page.setContent(`
        <div style="height: 2000px;"></div>
        <button onclick="window._clicked=true">click me</button>
      `);

      const error = await page.locator("button").click({ scroll: "none", timeout: 2000 }).catch((caught) => caught);

      expect(String(error?.message ?? error)).toContain("Element is outside of the viewport");
      expect(await page.evaluate("window._clicked")).toBeFalsy();
      expect(await page.evaluate(() => window.scrollY)).toBe(0);
    });
  });

  it("should click in-viewport element when scroll is \"none\"", async () => {
    await withBidiPage(async (page) => {
      await page.setContent(`
        <button onclick="window._clicked=true">click me</button>
        <div style="height: 2000px;"></div>
      `);

      await page.locator("button").click({ scroll: "none", timeout: 2000 });

      expect(await page.evaluate("window._clicked")).toBe(true);
      expect(await page.evaluate(() => window.scrollY)).toBe(0);
    });
  });

  it("should not scroll nested container when scroll is \"none\"", async () => {
    await withBidiPage(async (page) => {
      await page.setContent(`
        <div style="height: 100px; width: 100px; overflow-y: scroll;">
          <div style="height: 50px;">A</div>
          <div style="height: 50px;">B</div>
          <button style="height: 50px; width: 100px;" onclick="window._clicked=true">C</button>
        </div>
      `);
      const button = page.locator("button");

      const error = await button.click({ scroll: "none", timeout: 2000 }).catch((caught) => caught);

      expect(error).toBeTruthy();
      expect(await page.evaluate("window._clicked")).toBeFalsy();
      await button.click({ timeout: 2000 });
      expect(await page.evaluate("window._clicked")).toBe(true);
    });
  });

  it("should not scroll on hover when scroll is \"none\"", async () => {
    await withBidiPage(async (page) => {
      await page.setContent(`
        <div style="height: 2000px;"></div>
        <div onmouseover="window._hovered=true" style="width: 50px; height: 50px;">hover me</div>
      `);

      const error = await page.locator("div >> text=hover me").hover({ scroll: "none", timeout: 2000 }).catch((caught) => caught);

      expect(String(error?.message ?? error)).toContain("Element is outside of the viewport");
      expect(await page.evaluate("window._hovered")).toBeFalsy();
      expect(await page.evaluate(() => window.scrollY)).toBe(0);
    });
  });
});
