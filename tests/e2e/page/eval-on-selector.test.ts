...
describe("page eval on selector e2e", () => {
  ...
  it("should work with spaces in css attributes", async () => {
    await withPage(async (page) => {
      ...
      expect(await page.$("input[placeholder='Select date']")).toBeTruthy();
      expect(await page.$eval('[placeholder="Select date"]', (e) => (e as HTMLElement).outerHTML)).toBe(
        '<input placeholder="Select date">'
      );
    });
  });

  it("should work with quotes in css attributes", async () => {
    await withPage(async (page) => {
      await page.setContent('<div><input placeholder="Select&quot;date"></div>');
      expect(await page.$('[placeholder="Select\\"date"]')).toBeTruthy();
      expect(await page.$("[placeholder='Select\"date']")).toBeTruthy();
  ...
  it("should work with spaces in css attributes when missing", async () => {
    await withPage(async (page) => {
  ...
  it("should work with quotes in css attributes when missing", async () => {
    await withPage(async (page) => {
      const inputPromise = page.waitForSelector('[placeholder="Select\\"date"]');
      expect(await page.$('[placeholder="Select\\"date"]')).toBe(null);
      await page.setContent('<div><input placeholder="Select&quot;date"></div>');
